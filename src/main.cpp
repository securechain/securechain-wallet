...
if(nHeight < 500)
    ...
    // Subsidy is cut in half every 2.1 million blocks which will occur approximately every 4 years.
    nSubsidy >>= halvings;

    return nSubsidy + nFees;
}

static const int64_t nTargetTimespan = 500 * 60; // approx 8 hours
static const int64_t nTargetSpacing = 60; // 60 seconds
static const int64_t nInterval = nTargetTimespan / nTargetSpacing; // 500 blocks
...
bool IsInitialBlockDownload()
{
    ...
    return (GetTime() - nLastUpdate < 10 &&
            chainActive.Tip()->GetBlockTime() < GetTime() - 96 * 60 * 60);
}
...
void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (chainActive.Tip()->GetBlockWork() * 60).getuint256()))
    {
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // We define a condition which we should warn the user about as a fork of at least 70 blocks
    // who's tip is within 720 blocks (+/- 12 hours if no one mines it) of ours
    // We use 70 blocks rather arbitrarily as it represents just under 10% of sustained network
    // hash rate operating on the fork.
    // or a chain that is entirely longer than ours and invalid (note that this should be detected by both)
    // We define it this way because it allows us to only store the highest fork tip (+ base) which meets
    // the 70-block condition and from this always have the most-likely-to-cause-warning fork
    if (pfork && (!pindexBestForkTip || (pindexBestForkTip && pindexNewForkTip->nHeight > pindexBestForkTip->nHeight)) &&
            pindexNewForkTip->nChainWork - pfork->nChainWork > (pfork->GetBlockWork() * 70).getuint256() &&
            chainActive.Height() - pindexNewForkTip->nHeight < 720)
    {
