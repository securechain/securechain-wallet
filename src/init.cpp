...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "\n" + _("SSL options: (see the Securechain Wiki for SSL setup instructions)") + "\n";
...
#endif
    // Make sure only a single Securechain Wallet process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SecureCoin is probably already running."), strDataDir));
